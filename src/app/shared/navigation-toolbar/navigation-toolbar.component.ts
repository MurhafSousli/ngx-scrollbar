...
export class NavigationToolbarComponent implements AfterViewChecked, OnDestroy {
  ...
  examples: Observable<NavigationButtons[]> = this.state.pipe(distinctUntilChanged());
      ...
      this.mainScrolledSubscription = this.mainScrolled.pipe(
        ...
        map((e: any) => {
          ...
          return this.state.value.map((example: NavigationButtons) => {
            ...
            if (section && section.firstChild) {
              ...
              return {
                ...example,
                active: firstChild.offsetTop - 32 <= fromTop && firstChild.offsetTop + firstChild.offsetHeight + 32 > fromTop
              };
            }
            return undefined;
          });
        ...
        tap((examples: NavigationButtons[]) => {
          ...
          if (shouldScrollToElement) {
            this.ngZone.run(() => this.state.next(examples));
            this.navScrollbar.scrollToElement('.mat-tab-label-active', 0, 200);
          } else {
