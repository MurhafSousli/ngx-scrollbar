...
describe('Scrollbar track', () => {
  ...
  it('should not scroll when mouse is down and moves away', async () => {
    ...
    let clientY: number = trackRect.bottom + trackRect.height - thumbRect.height / 2;
    ...
    // Ongoing click
    await afterTimeout(124);
    expect(component.viewport.scrollTop).toBeGreaterThanOrEqual(200);

    // fake mouse out
    const scrollTopBeforeMouseOut: number = component.viewport.scrollTop;
    trackYDebugElement.nativeElement.dispatchEvent(new PointerEvent('pointerleave'));
    await afterTimeout(100);
    ...
    trackYDebugElement.nativeElement.dispatchEvent(new PointerEvent('pointerenter', { clientY }));
